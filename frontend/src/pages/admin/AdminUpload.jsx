...
import { Upload, CheckCircle2, XCircle, Database, Loader2, RefreshCw, Sliders, Languages, Users, ShieldCheck, Factory, Copy, Check, AtSign, IdCard, LayoutGrid, Sun, Moon, Megaphone, Trash2, ListChecks, ExternalLink, KeyRound } from "lucide-react";
...
const ADMIN_TABS = ["data", "production", "translations", "users", "profiles", "access", "broadcast", "cleanup", "ltasks", "permissions"];
    ...
    <div className="min-h-screen bg-[var(--bg-base)] text-[var(--text-1)] overflow-x-hidden">
      ...
      <div className="no-scrollbar px-6 pt-4 overflow-x-auto">
        <SegmentedToggle
          ...
          options={[["data", t("admin.tabData"), Database], ["production", t("admin.tabProduction"), Factory], ["translations", t("admin.tabTranslations"), Languages], ["users", t("admin.tabUsers"), Users], ["profiles", t("admin.tabProfiles"), IdCard], ["access", t("admin.tabAccess"), ShieldCheck], ["permissions", t("admin.tabPermissions"), KeyRound], ["broadcast", t("admin.tabBroadcast"), Megaphone], ["cleanup", t("admin.tabCleanup"), Trash2], ["ltasks", t("admin.tabLtasks"), ListChecks]].filter(([id]) => allowedTabs.includes(id)).map(([id, label, Icon]) => ({
            value: id,
      ...
      {adminTab === "ltasks" && <LeaderTasksAdmin />}
