  ...
  return (
    <div className="max-w-4xl mx-auto p-4 sm:p-8">
      <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-xl p-5">
        ...
        <div className="flex items-center justify-between mb-2">
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${
              ...
              saveStatus === "saving" ? "bg-[var(--bg-inner)] text-[var(--text-3)] border border-[var(--border-md)] cursor-not-allowed" :
        ...
        <p className="text-[11px] text-[var(--text-3)] mb-5">{t("admin.access.hint")}</p>
              ...
              <thead>
                <tr style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                  ...
                  {COLUMNS.map((role) => (
                    <th
                      key={role}
                      className="py-2 px-3 text-[10px] font-semibold text-[var(--text-3)] uppercase tracking-wider text-center whitespace-nowrap"
                    >
                      <span className="inline-flex items-center gap-1 justify-center">
                        {role === "admin" && <Lock size={9} className="text-[var(--text-4)]" />}
