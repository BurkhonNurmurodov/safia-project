"""
What is ARC's OLD login API (page /arc-legacy) actually willing to give us?
...
from app.services import arc_legacy_client

log = logging.getLogger(__name__)

REQUESTS_PATH = arc_legacy_client._REQUESTS_PATH
...
def run_probe(db, client: Optional[httpx.Client] = None) -> dict:
    ...
    try:
        ...
        base = arc_legacy_client.probe_requests(client)
        ...
        if params:
            ...
            for p in params:
                for value in _candidates(p):
                    if spent >= MAX_PROBES:
                        break
                    spent += 1
                    res = arc_legacy_client.probe_requests(client, {p["name"]: value})
                    total = res.get("total")
                    trials.append({
            ...
            for name in _CANDIDATE_NAMES:
                if spent >= MAX_PROBES - 12:
                    break
                spent += 1
                res = arc_legacy_client.probe_requests(client, {name: _ORACLE_VALUE})
                ...
                if value is None or spent >= MAX_PROBES:
                    continue
                spent += 1
                res2 = arc_legacy_client.probe_requests(client, {name: value})
        ...
        combined = None
        if winners:
            combined = arc_legacy_client.probe_requests(client, winners)
            spent += 1
        ...
        for path in _CANDIDATE_PATHS:
            if spent >= MAX_PROBES:
                break
            spent += 1
            extras[path] = arc_legacy_client.get_path(client, path, {"page": 1, "size": 1})
        ...
        report = {
            ...
            "token": arc_legacy_client.token_claims(client),
        ...
        if meta is not None:
            ...
            db.commit()
        log.info("arc-legacy probe: baseline=%s combined=%s filters=%s spec=%s calls=%s",
...
def probe_now() -> dict:
    ...
    try:
        ...
        log.warning("arc-legacy probe failed: %s", str(exc)[:300])
