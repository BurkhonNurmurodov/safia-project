"""
HTTP client for ARC's OLD login API (page /arc-legacy).

This is the client /arc itself ran on until 25 Aug 2026, when IT's internal
read-only API (services/arc_client.py) replaced it. Revived on 25 Sep 2026 as
a SEPARATE mirror: its own settings (arc_legacy_*), its own tables, its own
page — nothing here is shared with /arc.
...
def configured() -> bool:
    return bool(settings.arc_legacy_username and settings.arc_legacy_password)
...
def diagnostics() -> dict:
    ...
    return {
        ...
        "resolved": {"username": bool(settings.arc_legacy_username), "password": bool(settings.arc_legacy_password),
                     "api_url": _base()},
...
def _base() -> str:
    return (settings.arc_legacy_api_url or "").rstrip("/")
...
def _login(client: httpx.Client) -> str:
    ...
    log.debug("ARC-legacy login → %s", url)
    try:
        res = client.post(url, json={"username": settings.arc_legacy_username,
                                     "password": settings.arc_legacy_password},
                          timeout=_TIMEOUT)
...
def get_json(client: httpx.Client, path: str, params: Optional[dict] = None,
    ...
    url = _base() + path
    log.debug("ARC-legacy GET %s params=%s", url, params or {})
    try:
...
def iter_requests(client: httpx.Client, size: int = 100,
    ...
    ``extra`` is the discovered filter set (see services/arc_legacy_discovery.py) —
...
def fetch_openapi(client: httpx.Client) -> Optional[dict]:
    ...
    try:
        ...
        log.info("ARC-legacy openapi fetch skipped: %s", str(exc)[:200])
        return None
...
def normalize_item(item: dict) -> dict:
    """One API item → the ArcLegacyRequest column dict (``raw`` keeps the whole
