...
the profile's **ru** display override to the exact sheet spelling, so that
...
SHEET_LANG = "ru"  # profile display override that carries the Cyrillic sheet spelling
...
def sheet_name_map(db: Session, names: Iterable[str]) -> dict[str, str]:
    ...
    names, using each profile's ru display override as the sheet name."""
