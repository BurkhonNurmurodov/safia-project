"""
Excel export of the /arc-legacy register (ARC's OLD login API).
...
def build_arc_legacy_workbook(rows: list[dict], columns: Optional[list[str]] = None,
                       labels: Optional[dict[str, str]] = None) -> BytesIO:
    """One sheet «ARC legacy»: a bold header row, frozen at A2, one row per ticket
