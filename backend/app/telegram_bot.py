...
_ROLE_LABELS = {
    "uz":      {"idle-owner": "Kutish mas'uli", "top-manager": "Top-menejer", "shift-manager": "Smena menejeri", "supervisor": "Brigadir", "leader": "Lider", "admin": "Admin", "guest": "Mehmon"},
    "uz_cyrl": {"idle-owner": "Кутиш масъули", "top-manager": "Топ-менежер", "shift-manager": "Смена менежери", "supervisor": "Бригадир", "leader": "Лидер", "admin": "Админ", "guest": "Меҳмон"},
    "ru":      {"idle-owner": "Ответственный за ожидание", "top-manager": "Топ-менеджер", "shift-manager": "Сменный менеджер", "supervisor": "Бригадир", "leader": "Лидер", "admin": "Администратор", "guest": "Гость"},
    "en":      {"idle-owner": "Idle-time owner", "top-manager": "Top Manager", "shift-manager": "Shift Manager", "supervisor": "Supervisor", "leader": "Leader", "admin": "Admin", "guest": "Guest"},
}
    ...
    with SessionLocal() as db:
        ...
        if role == "supervisor":
            ...
            if not p:
                return _unresolved()
            role_id = p.id
        elif role == "idle-owner":
            # «Kutish mas'uli» — a pre-created profile claimed by name, exactly
            # like the two manager tiers. WHICH ojidaniya categories they own is
            # not asked here and never will be: the assignment is an admin's
            # decision on the «Kutish mas'ullari» destination, and a person
            # naming their own scope at registration is not an assignment.
            p = db.query(RoleProfile).filter_by(role="idle-owner", name=full_name).first()
            if not p:
                return _unresolved()
            role_id = p.id
        elif role == "guest":
