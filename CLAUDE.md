...
| Factory (plant) switcher | `FactoryTabs.jsx` | THE plant context strip for the six factory-aware pages. Renders as the FIRST child of `<Layout>`, above the toolbar row — a factory is the page's SUBJECT, not another filter, so it never joins the filter row. Full-bleed band (`-mx-4 md:-mx-6` + hairline rule) carrying a `Factory` icon chip, the noun «Zavod», and a `SegmentedToggle` (`asTabs` + `scrollable`); «All factories» is the LAST tab. Renders **nothing** when fewer than two factories exist. A locked viewer (supervisor/leader) gets the same band with a static chip instead of the toggle — never a one-option toggle. |
