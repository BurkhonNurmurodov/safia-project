...
| Segmented toggle (min/hrs, P·A·P−A, view/mode switch, theme) | `SegmentedToggle.jsx` | iOS-style sliding pill: a recessed `bg-inner` track (`rounded-xl`, 3px inset, no dividers) holding segments — the selected one is a RAISED `bg-card` pill (soft shadow + `text-1`), the rest transparent with muted `text-3`. No brand-gold fill. Outer height stays `size="md"` (default, = `Button` md 38px) or `"sm"` (= `Button` sm 30px) so it aligns in toolbars. `options` = `[value,label]` tuples or `{value,label,title}` objects (label may be a node/icon). Never hand-roll the bar. |
