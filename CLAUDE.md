...
| Column show/hide + reorder | `ColumnsPicker.jsx` | Notion-style: 38px `Columns3` icon trigger (toolbar row, hidden-count badge) + portaled panel with Shown/Hidden groups, eye toggles, Hide all/Show all, and drag-to-reorder that only arms via the panel's reorder button. Controlled: `columns [{key,label,locked}]`, `order`, `hidden`, `onChange({order,hidden})`. Persist via `/api/ui-prefs/{key}` (per-profile JSON blobs, `UiPref` model); reconcile saved keys against the current column catalog and keep identity columns `locked`. `t("cols.*")` keys exist in all 4 langs. See the Production «Позиции» table for the reference wiring (cells rendered by a per-key switch so hide/reorder is free). |
